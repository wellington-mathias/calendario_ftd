...
function validateUsername($usuario)
{
    if (!empty($usuario->login)) {
        ...
        if (!is_null($user)) {
            sendMessage(503, array("message" => "Unable to create usuario. Login already in use"));
        }
    }
    ...
    if (!empty($usuario->login_ftd)) {
        ...
        if (!is_null($user)) {
            sendMessage(503, array("message" => "Unable to create usuario. Login FTD already in use"));
        }
    }
}
