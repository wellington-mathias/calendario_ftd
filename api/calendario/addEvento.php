...
if ($data_incomplete) {
    sendMessage(400, array("message" => "Unable to add Evento to Calendario. Data is incomplete."));
} else {
    $obj = new Evento();
    ...
    if ($evento != null) {
        ...
        sendMessage(200, array("message" => "Evento already inserted."));
    } else {
        $evento = new Evento();
        ...
        if (!$evento->readOne()) {
            ...
            if (!$calendario->addEvento($evento->id)) {
                sendMessage(400, array("message" => "Can't add Evento. An error has been occurred."));
            } else {
                sendMessage(200, array("message" => "Evento added."));
            }
        }
    }
}
