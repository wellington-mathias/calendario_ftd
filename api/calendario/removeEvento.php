...
if ($data_incomplete) {
    sendMessage(400, array("message" => "Unable to remove Evento to Calendario. Data is incomplete."));
} else {
    $evento = new Evento();
    $evento->id = $data->evento_id;

    if (!$evento->readOne()) {
        sendMessage(404, array("message" => "Evento doesn't exist."));
    } else {
        $obj = new Evento();
        ...
        if ($evento == null) {
            ...
            sendMessage(200, array("message" => "Evento already removed."));
        } else {
            $calendario = new Calendario();
            $calendario->id = $data->id;

            if (!$calendario->removeEvento($data->evento_id)) {
                sendMessage(400, array("message" => "Can't remove Evento. An error has been occurred."));
            } else {
                sendMessage(200, array("message" => "Evento removed."));
            }
        }
    }
}
